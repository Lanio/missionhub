...
class Organization < ActiveRecord::Base
  ...
  def delete_person(person)
    ...
    if person.organizational_permissions_including_archived.where("organization_id <> ?", id).blank?
      ...
      organizational_permissions.where(person_id: person.id).update_all(deleted_at: Time.now)
  ...
  def remove_permission_from_person(person, permission_id)
    person_id = person.is_a?(Person) ? person.id : person
    OrganizationalPermission.where(person_id: person_id, organization_id: id, permission_id: permission_id).each { |r| r.update_attributes(deleted_at: Time.now) }
  end
